...
export default function Home() {
  ...
  const answerQuiz = (option: string) => { if (quizSelected) return; setQuizSelected(option); if (option === quizQuestions[quizIndex].answer) setQuizScore((score) => score + 1); };
  const nextQuiz = () => { setQuizSelected(null); setQuizIndex((index) => index + 1); };
  ...
  const renderAnatomy = () => (
    <ExhibitFrame eyebrow="FORM / STRUCTURE" title="ANATOMY" intro="A typeface is a system of relationships. Pull one apart." number="02">
      <div className="anatomy-layout">
        <div className="anatomy-stage">
          <div className="letter-controls"><span className="mono">LETTER /</span>{["g", "A", "R", "S", "e"].map((letter) => <button key={letter} className={anatomyLetter === letter ? "active" : ""} onClick={() => setAnatomyLetter(letter)}>{letter}</button>)}</div>
          <div className="anatomy-letter-wrap"><span className="measure-label cap">CAP HEIGHT</span><span className="measure-label xheight">X-HEIGHT</span><span className="measure-label base">BASELINE</span><span className="measure-label desc">DESCENDER</span><div className="guide-line guide-cap" /><div className="guide-line guide-x" /><div className="guide-line guide-base" /><div className="guide-line guide-desc" /><div className="anatomy-letter" style={{ fontFamily: anatomyLetter === "g" ? "'Fraunces', serif" : "'Playfair Display', serif" }}>{anatomyLetter}</div><div className="anatomy-points"><i className={anatomyTerm === "Counter" ? "active" : ""} style={{ left: "44%", top: "57%" }} /><i className={anatomyTerm === "Bowl" ? "active" : ""} style={{ left: "64%", top: "32%" }} /><i className={anatomyTerm === "Stem" ? "active" : ""} style={{ left: "26%", top: "28%" }} /></div></div>
          <div className="anatomy-caption"><span className="mono">SPECIMEN / {anatomyLetter.toUpperCase()} / FRAUNCES</span><p>Selected structure <strong>{anatomyTerm}</strong> is highlighted in electric blue.</p></div>
        </div>
        <aside className="term-index"><div className="term-index-head"><span className="eyebrow">TERMINOLOGY INDEX</span><span className="mono">{anatomyTerms.findIndex((term) => term.label === anatomyTerm) + 1} / {anatomyTerms.length}</span></div><div className="term-list">{anatomyTerms.map((term) => <button key={term.label} className={anatomyTerm === term.label ? "active" : ""} onClick={() => setAnatomyTerm(term.label)}><span>{term.label}</span><ArrowUpRight size={14} /></button>)}</div><div className="term-definition"><span className="mono">OBSERVATION</span><h3>{anatomyTerm}</h3><p>{anatomyTerms.find((term) => term.label === anatomyTerm)?.note}</p></div></aside>
      </div>
    </ExhibitFrame>
  );
  ...
  const renderPsychology = () => (
    <ExhibitFrame eyebrow="TONE / PERCEPTION" title="PSYCHOLOGY" intro="The words stay the same. The voice does not." number="04">
      <div className="psychology-layout"><div className="mood-list"><span className="mono">CHOOSE AN IMPRESSION</span>{Object.entries(psychologyFaces).map(([key, value]) => <button key={key} className={mood === key ? "active" : ""} onClick={() => setMood(key)}><span className="mood-dot" style={{ background: value.color }} />{value.label}<ArrowUpRight size={15} /></button>)}<div className="mood-note"><BookOpen size={17} /><p>Type is not neutral. Shape, rhythm and historical memory all change how a message lands.</p></div></div><div className="psychology-stage" style={{ background: selectedMood.color }}><span className="stage-corner dark-corner">MESSAGE / 001</span><div className="psych-message" style={{ fontFamily: selectedMood.font }}><span>We need</span><strong>your attention.</strong></div><div className="psych-footer"><span>{selectedMood.label}</span><p>{selectedMood.body}</p></div></div></div>
    </ExhibitFrame>
  );
  ...
  return (
    <div className={`lab-shell ${theme} ${grid ? "show-grid" : ""} ${exhibition ? "is-exhibition" : ""}`} onPointerMove={handlePointer} style={{ "--pointer-x": `${pointer.x}%`, "--pointer-y": `${pointer.y}%` } as React.CSSProperties}>
      {!entered ? <section className="entry-screen"><div className="entry-grid" /><div className="entry-top"><span>TYPE//LAB</span><span>AN INTERACTIVE STUDY OF TYPOGRAPHY</span><span>EST. 2026 / 01—09</span></div><div className="entry-center"><div className="entry-marker"><span className="blue-dot" /> A DIGITAL EXHIBITION</div><h1><span>TYPE</span><span>IS</span><span>EVERYWHERE<span className="period">.</span></span></h1><div className="entry-bottom"><p>Typography is not decoration.<br />It is how information gets a body.</p><button className="enter-button" onClick={() => setEntered(true)}>ENTER THE LAB <ArrowRight size={17} /></button></div></div><div className="entry-cursor">MOVE TO BEGIN <MousePointer2 size={14} /></div></section> : <>
        ...
        {toolbox && <aside className="toolbox-panel"><div className="toolbox-head"><span><span className="blue-dot" /> TYPE TOOLBOX</span><button aria-label="Close toolbox" onClick={() => setToolbox(false)}><X size={16} /></button></div><p className="toolbox-intro">Adjust the instrument. Your settings stay with the lab.</p><div className="toolbox-section"><span className="mono">VIEW</span><button className="toolbox-control" onClick={() => setTheme(theme === "light" ? "dark" : "light")}><span>{theme === "light" ? <Moon size={16} /> : <Sun size={16} />} {theme === "light" ? "Dark mode" : "Light mode"}</span><kbd>{theme === "light" ? "OFF" : "ON"}</kbd></button><button className="toolbox-control" onClick={() => setGrid(!grid)}><span><Grid3X3 size={16} /> Grid overlay</span><kbd>{grid ? "ON" : "OFF"}</kbd></button><button className="toolbox-control" onClick={() => setExhibition(!exhibition)}><span><Maximize2 size={16} /> Exhibition mode</span><kbd>↗</kbd></button></div><div className="toolbox-section"><span className="mono">QUICK SPECIMEN</span><RangeControl label="Size" value={machineSize} min={28} max={150} onChange={setMachineSize} suffix="px" /><RangeControl label="Spacing" value={machineSpacing} min={-5} max={12} step={0.5} onChange={setMachineSpacing} suffix="px" /><label className="field-label">TYPEFACE<select className="lab-select" value={machineFont} onChange={(event) => setMachineFont(event.target.value)}>{fontOptions.map((font) => <option key={font.value} value={font.value}>{font.label}</option>)}</select></label></div><button className="toolbox-reset" onClick={resetAll}><RotateCcw size={15} /> Reset lab settings</button><div className="toolbox-footer"><Keyboard size={15} /><span>Esc closes panels · ← → move between exhibits</span></div></aside>}
