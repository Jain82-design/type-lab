...
function RangeControl({ label, value, min, max, step = 1, onChange, suffix = "" }: { label: string; value: number; min: number; max: number; step?: number; onChange: (value: number) => void; suffix?: string }) {
  return (
    <label className="range-control">
      <span><span className="feature-label">{label}</span><b>{value}{suffix}</b></span>
...
export default function Home() {
  ...
  const [machineBoldness, setMachineBoldness] = useState(50);
  ...
  const [compareLeading, setCompareLeading] = useState(1.15);
  ...
  const [serifReadLeading, setSerifReadLeading] = useState(1.4);
  ...
  const resetMachine = () => { setMachineText("Typography is a voice."); setMachineFont(fontOptions[0].value); setMachineSize(74); setMachineWeight(600); setMachineBoldness(50); setMachineSpacing(-1); setMachineLeading(1.05); setMachineAlign("left"); setMachineCase("none"); setMachineRotate(0); };
  ...
  const renderMachine = () => {
    ...
    const applyPreset = (preset: string) => { const selected = presets[preset]; setMachinePreset(preset); setMachineFont(selected.font); setMachineSize(selected.size); setMachineWeight(selected.weight); setMachineBoldness(50); setMachineSpacing(selected.spacing); setMachineLeading(selected.leading); setMachineRotate(selected.rotation); setMachineScale(selected.scale); setMachineOpacity(selected.opacity); setMachineInk(selected.ink); };
  ...
  const renderTypeTest = () => {
    ...
    return <div className={`type-test-panel ${typeTestFullscreen ? "full-type-test" : ""}`}>
      ...
      <div className="type-test-grid"><div className="type-test-stage"><label className="character-input"><span className="feature-label">CHARACTER <b>{typeTestText.length} CHARS</b></span><textarea aria-label="Character input" placeholder="Aa / Type here…" value={typeTestText} onChange={(event) => setTypeTestText(event.target.value)} /></label><div className={`type-test-specimen case-${typeTestCase}`} style={{ fontFamily: typeTestFont, fontSize: `${typeTestSize}px`, fontWeight: typeTestWeight, letterSpacing: `${typeTestSpacing}px`, lineHeight: typeTestLeading, textAlign: typeTestAlign as React.CSSProperties["textAlign"], color: typeTestInk === "blue" ? "var(--blue)" : "var(--ink)", fontStyle: typeTestItalic ? "italic" : "normal" }}>{typeTestText || "Type something."}</div><div className="type-test-readout"><span>FONT / {fontOptions.find((font) => font.value === typeTestFont)?.label}</span><span>SIZE / {typeTestSize}px</span><span>WEIGHT / {typeTestWeight}</span><span>TRACK / {typeTestSpacing}px</span></div></div><div className="type-test-controls"><label className="field-label feature-label">SPECIMEN A<select className="lab-select" value={typeTestFont} onChange={(event) => setTypeTestFont(event.target.value)}>{fontOptions.map((font) => <option key={font.value} value={font.value}>{font.label}</option>)}</select></label><label className="field-label feature-label">SPECIMEN B<select className="lab-select" value={typeTestFontB} onChange={(event) => setTypeTestFontB(event.target.value)}>{fontOptions.map((font) => <option key={font.value} value={font.value}>{font.label}</option>)}</select></label><RangeControl label="Font size" value={typeTestSize} min={28} max={150} onChange={setTypeTestSize} suffix="px" /><RangeControl label="Weight" value={typeTestWeight} min={300} max={900} step={100} onChange={setTypeTestWeight} /><RangeControl label="Tracking" value={typeTestSpacing} min={-5} max={12} step={0.5} onChange={setTypeTestSpacing} suffix="px" /><RangeControl label="Leading" value={typeTestLeading} min={0.75} max={1.8} step={0.05} onChange={setTypeTestLeading} /><div className="control-row"><label className="field-label compact feature-label">ALIGN<select className="lab-select" value={typeTestAlign} onChange={(event) => setTypeTestAlign(event.target.value)}><option value="left">Left</option><option value="center">Center</option><option value="right">Right</option></select></label><label className="field-label compact feature-label">CASE<select className="lab-select" value={typeTestCase} onChange={(event) => setTypeTestCase(event.target.value)}><option value="none">None</option><option value="uppercase">Upper</option><option value="lowercase">Lower</option></select></label></div><div className="toolbox-subcontrols"><label className="field-label compact feature-label">INK<select className="lab-select" value={typeTestInk} onChange={(event) => setTypeTestInk(event.target.value)}><option value="black">Black</option><option value="blue">Blue</option></select></label><button className={`toggle-lab ${typeTestItalic ? "active" : ""}`} onClick={() => setTypeTestItalic(!typeTestItalic)}>Italic where supported</button></div><div className="preset-grid"><span className="mono feature-label">PRESETS</span>{Object.keys(presets).map((preset) => <button key={preset} className={typeTestPreset === preset ? "active" : ""} onClick={() => applyPreset(preset)}>{preset}</button>)}</div><div className="toolbox-bottom-actions"><button className="text-button" onClick={resetTypeTest}><RotateCcw size={14} /> Reset</button><button className="text-button" onClick={() => navigator.clipboard?.writeText(`font-family: ${typeTestFont};\nfont-size: ${typeTestSize}px;`)}><Copy size={14} /> Copy CSS</button><button className="text-button" onClick={() => localStorage.setItem("type-lab-saved-specimen", typeTestText)}><Save size={14} /> Save</button></div></div></div>
